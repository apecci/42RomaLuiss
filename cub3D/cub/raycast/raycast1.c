...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/09/07 14:27:47 by apecci            #+#    #+#             */
/*   Updated: 2021/09/07 14:27:48 by apecci           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ver_line(int x, t_caster *caster, t_window *win, t_data *img)
{
	...
	while (++i < caster->draw_start)
	{
		...
		else
		{
			...
			my_mlx_pixel_put(img, x, i, tex->addr[(int)(win->plr->tilt
					* tex->img_w) + i * tex->img_w + x]);
		}
	}
